...
namespace NexusStrap
{
    internal class Installer
    {
        ...
        public bool ImportSettings =
            Directory.Exists(Path.Combine(Paths.LocalAppData, "NexusStrap")) ||
        ...
        public void DoInstall()
        {
            ...
            // only register player, for the scenario where the user installs nexusstrap, closes it,
        ...
        public void DeployApplicationPayload()
        {
            const string LOG_IDENT = "Installer::DeployApplicationPayload";

            string? sourceDir = Path.GetDirectoryName(Paths.Process);
        ...
        private bool ValidateLocation()
        {
            ...
            // prevent issues with settings importing - don't install into other bootstrappers' data folders
            string[] otherDataFolders = { "Fishstrap", "Lunastrap", "Luczystrap" };

            foreach (var folder in otherDataFolders)
            {
                string otherDir = Path.Combine(Paths.LocalAppData, folder);

                if (InstallLocation.Equals(otherDir, StringComparison.InvariantCultureIgnoreCase)
                    || InstallLocation.StartsWith(otherDir + Path.DirectorySeparatorChar, StringComparison.InvariantCultureIgnoreCase))
                    return false;
            }
        ...
        public static void DoUninstall(bool keepData)
        {
            ...
            if (!keepData)
            {
                ...
                // Only delete the NexusStrap folder if keepData is false
        ...
        public static void HandleUpgrade()
        {
            ...
            var upgradeInstaller = new Installer { InstallLocation = Paths.Base };

            for (int i = 1; i <= 10; i++)
            {
                try
                {
                    upgradeInstaller.DeployApplicationPayload();
                    break;
                }
                catch (Exception ex)
                {
            ...
            if (existingVer is not null)
            {
                ...
                if (Utilities.CompareVersions(existingVer, "1.4.2") == VersionComparison.LessThan)
                {
                    ...
                    string genCacheDir = Path.Combine(Path.GetTempPath(), "NexusStrap", "mod-generator");
                    string pluginCacheDir = Path.Combine(Paths.Roblox, "Plugins", "NexusStrapStudioRPC.rbxmx");
...
#pragma warning disable CS0162 // Unreachable code detected
                if (false)
                    Utilities.ShellExecute($"https://github.com/{App.ProjectRepository}/wiki/Release-notes-for-NexusStrap-v{currentVer}");
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public void ImportSettingsFromSelectedApp()
        {
            ...
            {
                ImportSettingsFrom.NexusStrap => Path.Combine(Paths.LocalAppData, "NexusStrap"),
            ...
            // installing into the same folder we'd import from - nothing to import
            if (sourceDir.Equals(InstallLocation, StringComparison.InvariantCultureIgnoreCase))
                return;
