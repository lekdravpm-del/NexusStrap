...
namespace NexusStrap.Integrations
{
    ...
    public class AccountManager
    {
        ...
        private static readonly byte[] DpapiEntropy = Encoding.UTF8.GetBytes("NexusStrap_DPAPI_v1");
        ...
        public async Task<AltAccount?> AddAccountByQuickSignInAsync()
        {
            ...
            try
            {
                ...
                App.RichPresence?.SetDialog("Quick Sign-In");
        ...
        /// <summary>
        /// Looks for an already installed Chromium-based browser (Chrome, Edge or Brave)
        /// so the account manager doesn't require a standalone Chromium download.
        /// </summary>
        private static string? DetectSystemBrowser()
        {
            string[] candidates =
            {
                // Google Chrome
        ...
        public async Task<AltAccount?> AddAccountByBrowser()
        {
            ...
            try
            {
                ...
                if (executablePath == null)
                {
                    ...
                    if (executablePath != null)
                        App.Logger.WriteLine(LOG_IDENT_BROWSER, $"Using installed Chromium-based browser: {executablePath}");
                }
